...
## Hygiene
- Never commit scratch / debug scripts (`check_*.py`, `fix_*.py`, `verify_*.py`, `regen*.py`, `test_api.py`, `test_detail.py`) or local backups (`backup.json`, `printedge_backup.json`, `model_*.txt`) or temp markers (`.chunk*.done`, `write_probe.txt`, `nul`). They are gitignored — remove them before committing.
